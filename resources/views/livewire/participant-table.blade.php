        ...
        <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex gap-3">
                ...
                <button
                    ...
                    class="flex items-center gap-2 px-4 py-2.5 bg-secondary text-white text-sm font-semibold rounded-xl hover:bg-green-700 transition-all shadow-sm disabled:opacity-60"
                    title="Download Excel">
                    <span wire:loading.remove wire:target="downloadExcel">
                        <i class="fas fa-file-export text-xs"></i>
                        <span class="hidden sm:inline ml-1">Export Excel</span>
                    </span>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group" wire:key="participant-{{ $p->id }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex flex-col items-center gap-1">
                                ...
                                {{-- LAYANAN SELECT --}}
                                <select
                                    wire:change="updateField('{{ $p->id }}', 'layanan_id', $event.target.value)"
                                    class="w-82.5 text-[11px] font-semibold bg-transparent border-none focus:ring-0 p-0 cursor-pointer text-slate-600 hover:text-primary text-center">
                                    <option value="">-</option>
                                    @foreach($services as $s)
                                    <option value="{{ $s->id }}" {{ $p->layanan_id == $s->id ? 'selected' : '' }}>
                                        {{ $s->nama }}
                                    </option>
                                    @endforeach
                                </select>
                                ...
                                {{-- SEGMENT SELECT --}}
                                <select
                                    wire:change="updateField('{{ $p->id }}', 'segment_id', $event.target.value)"
                                    class="w-82.5 text-[11px] font-semibold bg-transparent border-none focus:ring-0 p-0 cursor-pointer text-slate-600 hover:text-primary text-center">
                                    <option value="">-</option>
                                    @foreach($segments as $seg)
                                    <option value="{{ $seg->id }}" {{ $p->segment_id == $seg->id ? 'selected' : '' }}>
                                        {{ $seg->nama }}
                                    </option>
                                    @endforeach
                                </select>
                                ...
                                <span class="text-xs font-bold text-slate-700 max-w-75 truncate block">
                                    {{ $p->service->nama ?? '-' }}
                                </span>
                                <span class="text-[10px] font-bold text-slate-400 uppercase max-w-75 truncate block">
                                    {{ $p->segment?->nama ?? '-' }}
                                </span>
                            ...
                            <div class="flex flex-col gap-2 text-[11px] items-center">
                                ...
                                <div class="w-full max-w-35 flex items-center gap-2 px-2 py-1.5 rounded-md bg-blue-50 border border-blue-100 justify-center">
                                    ...
                                    <div class="flex flex-col items-center leading-tight w-full">
                                        <span class="text-[9px] font-bold text-blue-500 uppercase tracking-wider text-center">
                                            Input
                                        </span>
                                        <span class="font-semibold text-slate-700 truncate w-full text-center">
                                            {{ $p->creator->nama ?? '-' }}
                                        </span>
                                    </div>
                                ...
                                <div class="w-full max-w-35 flex items-center gap-2 px-2 py-1.5 rounded-md bg-green-50 border border-green-100 justify-center">
                                    ...
                                    <div class="flex flex-col items-center leading-tight w-full">
                                        <span class="text-[9px] font-bold text-green-500 uppercase tracking-wider text-center">
                                            Proses
                                        </span>
                                        <span class="font-semibold text-slate-700 truncate w-full text-center">
                                            {{ $p->processor->nama ?? '-' }}
                        ...
                        <td class="px-6 py-4 text-center whitespace-nowrap">
                            <div class="flex justify-end gap-2">
                                ...
                                {{-- WA — hanya admin/super_admin --}}
                                @if($p->status === 'pending' && $isAdminOrSuper)
                                <button
                                    wire:click="sendWa('{{ $p->id }}')"
                                    wire:confirm="Proses peserta ini? WA notifikasi akan dikirim ke peserta."
                                    wire:loading.attr="disabled"
                                    ...
                                    title="Kirim WA">
                                    <span wire:loading wire:target="sendWa('{{ $p->id }}')">
                                        <i class="fas fa-spinner fa-spin"></i>
                                    </span>
                                    <span wire:loading.remove wire:target="sendWa('{{ $p->id }}')">
                                        <i class="fab fa-whatsapp text-xs"></i>
                                    </span>
                                </button>
                                @endif

                                {{-- PROSES — hanya admin/super_admin --}}
                                @if($p->status === 'pending' && $isAdminOrSuper)
                                <button
                                    wire:click="process('{{ $p->id }}')"
                                    wire:confirm="Proses peserta ini? WA notifikasi akan dikirim ke peserta."
                                    wire:loading.attr="disabled"
                                    class="w-8 h-8 flex items-center justify-center rounded-lg bg-secondary/10 text-secondary hover:bg-secondary hover:text-white transition-all shadow-sm disabled:opacity-50"
                                    title="Proses Data">
                                    <span wire:loading wire:target="process('{{ $p->id }}')">
                                        <i class="fas fa-spinner fa-spin"></i>
                                    </span>
                                    <span wire:loading.remove wire:target="process('{{ $p->id }}')">
                                        <i class="fas fa-check text-xs"></i>
                                    </span>
                                </button>
                                @endif

                                {{-- EDIT — admin/super_admin + pic (data sendiri) --}}
                                @if($p->status === 'pending')
                                <a
                                ...
                                {{-- HAPUS — admin/super_admin + pic (data sendiri) --}}
                                @if($p->status === 'pending')
                                <button
                                    wire:click="delete('{{ $p->id }}')"
                                    wire:confirm="Hapus data peserta ini?"
                                    wire:loading.attr="disabled"
                                    class="w-8 h-8 flex items-center justify-center rounded-lg bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm disabled:opacity-50"
                                    title="Hapus">
